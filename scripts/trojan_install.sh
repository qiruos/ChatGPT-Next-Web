...
# 域名
your_domain="www.qiruos.com"
# 名称
local_name="JP"
# 邮箱
your_email="qiruos@outlook.com"
...
function create_trojan_service() {
...
function checkDomain() {
  ...
  local_addr=$(curl -L -s https://ipinfo.io|grep "\"ip\":" |grep -Eo "[0-9\.]+")
  if [ "$real_addr" != "$local_addr" ] ; then
    error "域名解析地址($real_addr)与本VPS IP($local_addr)地址不一致" "本次安装失败，请确保域名解析正常"
    return 1
  fi
  return 0
}

function buildNginxSever() {
  systemctl enable trojan.service
  create_nginx_config
  rm -rf /usr/share/nginx/html/*
  cd /usr/share/nginx/html/ || return
  wget https://github.com/qiruos/ChatGPT-Next-Web/releases/download/v1.0.1/web.zip
  unzip web.zip
  # android
  wget -O /usr/share/nginx/html/clash.apk https://github.com/qiruos/ChatGPT-Next-Web/releases/download/v1.0.1/android-2.5.12-foss-arm64-v8a-release.apk
  # windows
  wget -O /usr/share/nginx/html/clash.exe https://github.com/qiruos/ChatGPT-Next-Web/releases/download/v1.0.1/windows.Setup.0.20.39.exe
  sleep 5
}

function installAcme() {
  if test -s ~/.acme.sh/acme.sh; then
     return 0
  fi
...
function updateHttpsCert() {
  # 安装 acme
  installAcme
  # 检查证书
  if checkHttpsCert; then
    return 0
  fi

  systemctl stop nginx
  mkdir /usr/src/trojan-cert /usr/src/trojan-temp > /dev/null 2>&1
...
function checkTrojanInstall() {
  processCount=$(ps -ef |grep -v  grep|grep -c trojan)
...
function buildTrojanServer() {
    cd /usr/src || return 1
    wget https://github.com/qiruos/ChatGPT-Next-Web/releases/download/v1.0.1/server-1.16.0-linux-amd64.tar.xz
...
function install_trojan(){
  try "重复安装检查" checkTrojanInstall
  try "系统检查" checkSystem
  try "安装依赖" installDependency
  try "SELINUX检查" checkSelinux
  try "域名检查" checkDomain
  systemctl stop nginx
  try "端口80检查" checkPortExist 80
  try "端口443检查" checkPortExist 443
  try "申请https证书" updateHttpsCert
  try "构建nginx服务" buildNginxSever
  try "构建trojan服务" buildTrojanServer
  try "启动nginx"  systemctl start nginx
  try "启动trojan" systemctl restart trojan
  green "======================================================================"
  green "Trojan已安装完成"
